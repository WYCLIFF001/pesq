...
/// 1.3.1: start `S = MARGIN_SAMPLES`, length
/// `n = buffer.len() - 2 * MARGIN_SAMPLES`.
///
...
pub fn apply_filter_curve(buffer: &mut [f32], curve: &Curve) {
    let start = MARGIN_SAMPLES;
    let n = buffer.len() - 2 * MARGIN_SAMPLES;
...
mod tests {
    ...
    fn alignment_filter_passes_the_passband_and_cuts_the_stopband() {
        ...
        apply_filter_curve(&mut filtered_pass, &ALIGNMENT_CURVE);
        apply_filter_curve(&mut filtered_stop, &ALIGNMENT_CURVE);
    ...
    fn irs_filter_is_unity_at_1_khz_and_shapes_the_edges() {
        ...
        apply_filter_curve(&mut filtered, &IRS_RECEIVE_CURVE);
        let energy = |buffer: &[f32]| buffer.iter().map(|s| f64::from(*s * *s)).sum::<f64>();
