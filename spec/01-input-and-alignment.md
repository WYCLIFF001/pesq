...
3. Set the reference cursor to s0*W and the degraded cursor to s0*W + d0. If the degraded cursor is negative, add -d0 to the reference cursor and set the degraded cursor to 0.
...
1. Let f1 = floor(((start[u] - 75)*W + j1)/128) and j = floor(((end[u-1] - 75)*W + j0)/128). If f1 > j, set f1 = j. If f1 < 0, set f1 = 0.
