...
4. The expected values above were produced by the reference model running natively at 8 kHz on the WAV samples as stored. The harness must therefore feed the original 8 kHz samples into the model, not a resampled copy. The crate models 8 kHz natively (spec 01, table 1.1), so it should expose an 8 kHz entry point for this purpose; the earlier harness workaround (upsampling the 8 kHz WAV to 16 kHz by linear interpolation and letting the public entry point decimate back by pair averaging) was a lossy round trip whose net effect is the filter y[n] = 0.75 x[n] + 0.25 x[n+1], about 6 dB attenuation at Nyquist, that no part of the specification defines. Measured impact on the conformance scores was small (about 0.01 of the raw score on pair 27), but the round trip was an unneeded transformation: use the 8 kHz entry point. The 16 kHz entry point remains, documented as decimating with a proper anti-aliasing filter (Round 4 replaced the provisional pair averaging with a short windowed-sinc decimator; see the crate docs of the decimation function).
