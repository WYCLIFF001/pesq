...
4. Powers: let R be the smallest power of two >= 2m. p1 = sum over i in [0, m) of |ref_seg[i]|^2, divided by R. Likewise p2 for the degraded segment. norm = sqrt(p1 * p2). If p1 <= 1e-6 or p2 <= 1e-6, the correlation result is 0 and the delay is 0.
5. Normalized circular cross-correlation at period R: h(tau) = | sum over i in [0, m) of |ref_seg[i]| * |deg_seg[(i + tau) mod R]| | / norm. Compute with FFTs: fill two length-R buffers with the absolute values of the first m samples of each segment, forward real FFT, divide the first buffer's bins (0..R/2) by R, binwise conjugate of the first times the second, inverse real FFT; h(tau) at a wrapped position is the absolute value of the output divided by norm.
